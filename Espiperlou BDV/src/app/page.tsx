  ...
  const handleOverlaySubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message = payload?.error ?? "No se pudo enviar la información. Intenta nuevamente.";
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "No se pudo enviar la información. Intenta nuevamente.";
